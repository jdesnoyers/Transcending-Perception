...
public class CameraProcessing : MonoBehaviour {
  ...
  void Update() {
    ...
    */
    ...
    if (_requestedImage.IsComplete) {
      //System.Array.Copy (packedImages, 0, l_image_data, 0, l_image_data.Length);
      //System.Array.Copy (packedImages, _requestedImage.Width * _requestedImage.Height, r_image_data, 0, r_image_data.Length);
    ...
    for (int i = (Width * Height) - 1; i > 0; i--) {
      if ((PixelsFound < PixelLimit) && (r_image_data[i] > brightnessThreshold)) {
    ...
    int lbiggest = 0;
    for (int i = (Width * Height) - 1; i > 0; i--) {
      if ((PixelsFound < PixelLimit) && (l_image_data[i] > brightnessThreshold)) {
...

    {
      ...
      for (int i = 0; i < rcount * 3; i = i + 3) {
        ...
        RBlobs[i] = -fBlob.x * RBlobs[i + 2];///xscalar;
        RBlobs[i + 1] = -fBlob.y * RBlobs[i + 2];///yscalar;
      }
      ...
      for (int i = 0; i < lcount * 3; i = i + 3) {
        ...
        LBlobs[i] = -fBlob.x * LBlobs[i + 2];///xscalar;
        LBlobs[i + 1] = -fBlob.y * LBlobs[i + 2];///yscalar;
      }
  ...
  private IEnumerator CheckImageMode() {
    ...
    leap_provider.GetLeapController().Config.Get<Int32>("images_mode", delegate(Int32 enabled) {
